...
class ICUAssistantAgent:
    ...
    def __init__(self) -> None:
        ...
        self.ingestion = ICUDataIngestionAgent()
        self.memory = ICUMemoryAgent()

    def load_patient(self, patient_id: str) -> None:
        path = f"database/icu_patients/{patient_id}.json"
        self.ingestion.load_patient(path)
        # align ingestion patient_id with given
        if self.ingestion.patient_id is None:
            self.ingestion.set_patient_id(patient_id)

    def replay_updates(self, num_updates: int) -> int:
        pid = self.ingestion.patient_id
        if not pid:
            raise ValueError("Patient not loaded")
        total_written = 0
        for i in range(1, num_updates + 1):
            batch = self.ingestion.update()
            if not batch:
                self.logger.info(f"No more events at update {i}; stop.")
                break
            ids = self.memory.add_events(pid, batch)
    ...
    def search_after_last(self, top_k: int = 20, window_hours: int = 24, tau_hours: float = 6.0) -> List[dict]:
        ...
        if not pid:
            raise ValueError("Patient not loaded")
        # get latest event id in memory
        latest = self.memory.get_recent_events(pid, n=1)
        if not latest:
            return []
        ev_id = latest[0]["id"]
        return self.memory.search_related_events(
            patient_id=pid,
            event_id=ev_id,
            top_k=top_k,
